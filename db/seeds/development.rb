...
Brand.create!([{name: 'シャネル'},{name: 'ルイヴィトン'},{name: 'シュプリーム'},{name: 'ナイキ'},{name: 'アディダス'}])

ItemStatus.create!([{ status: '新品、未使用'},{ status: '未使用に近い'},{ status: '目立った傷や汚れなし'},{ status: 'やや傷や汚れあり'},{ status: '傷や汚れあり'},{ status: '全体的に状態が悪い'}])

DeliveryMethod.create!([{method: '未定'},{method: 'らくらくメルカリ便'},{method: 'ゆうメール'},{method: 'レターパック'},{method: '普通郵便(定形、定形外)'},{method: 'クロネコヤマト'},{method: 'ゆうパック'},{method: 'クリックポスト'},{method: 'ゆうパケット'}])

DeliveryStatus.create!([{status: '1~2日で発送'},{status: '2~3日で発送'},{status: '4~7日で発送'}])
